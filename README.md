...
Built to demonstrate end-to-end data engineering: serverless ingestion, streaming Auto Loader, protobuf
decoding in-engine, SCD2 dimensions, parameterised job fan-out, and Infrastructure-as-Code — all
parametrised for dev/prod and deployed via the Databricks Asset Bundle.

> **Deeper docs:** operational quick-reference → [CLAUDE.md](CLAUDE.md) · design rationale →
> [docs/DESIGN_DECISIONS.md](docs/DESIGN_DECISIONS.md) · gold/analytics plan →
> [docs/GOLD_PLAN.md](docs/GOLD_PLAN.md).
...
The end goal (in progress) is a **gold** layer answering: *what's our on-time performance, how reliable
is the service, where and when does it fail?* — see [docs/GOLD_PLAN.md](docs/GOLD_PLAN.md).
...
GTFS-Realtime  (trip updates + vehicle positions)
  Azure Function (timer)  ──►  ADLS landing (.pb bytes)  ──►  Bronze (binaryFile)  ──►  Silver (from_protobuf)  ──►  Gold
  ingestion/function_app.py    lakehouse@transportvicdatalake    raw content column     explode → typed             (in progress)

GTFS-Static  (weekly schedule)
  Databricks job  ──►  extract zip  ──►  gtfs_data Volume  ──►  Bronze (CSV fan-out, 11 files)  ──►  Silver dims  ──►  Gold
  00_extractions/*     per-mode .txt      /Volumes/.../gtfs_data   parameterised for_each task      SCD2 + snapshots
```
...
| Gold | `fct_service_performance` + dims/KPIs | silver | *(in progress)* | — |
...
transport_victoria_databricks/ Databricks Asset Bundle
  ...
  src/.../03_gold/             analytics (in progress)
...
- 🔲 **Gold** — `fct_service_performance` star + executive KPIs ([docs/GOLD_PLAN.md](docs/GOLD_PLAN.md))
- 🔲 Dashboard (Power BI): on-time performance, reliability, delay hotspots, live vehicle map
